...
def predict():
    ...
    # youtube_id = str(int_features[0])
    # limit = int(int_features[1])

    youtube_id = request.json['url']
    ...
    res = scraper(youtube_id,limit)
    #fin = jsonify(res)
    my_test = pd.DataFrame(res)
    my_test.columns = ["comment"]
    t = my_test["comment"]
    my_test_p = vect.transform(t)
    prediction = pd.DataFrame(model.predict(my_test_p))
    my_test["sentiment"] = prediction
    # return render_template('index.html', tables=[my_test.to_html(classes='data')], titles=my_test.columns.values)
    my_test.to_csv('comment.csv', index=False, header=True)
    print(my_test.to_json(orient='records'))
    return my_test.to_json(orient='records')
